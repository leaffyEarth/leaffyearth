...
import { Plant, PlantTagsType } from "@leaffyearth/utils";
import { PlantResponseData, PlantsResponseData } from "@leaffyearth/utils/src/types/response.types";
  ...
  const fetchTopInteriorPlants = async () => {
    try {
      const response = await api.get(
        `/plants?tags=${PlantTagsType.TOP_INTERIOR}`,
      );
      ...
      <Grid sx={{ flexGrow: 1 }} container spacing={2}>
        {plants?.map((plant: Plant) => (
          <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={plant._id}>
            <PlantProductInCatalog
              Plant={plant}
              key={plant._id}
              onChange={fetchTopInteriorPlants}
            />
          </Grid>
        ))}
      </Grid>
