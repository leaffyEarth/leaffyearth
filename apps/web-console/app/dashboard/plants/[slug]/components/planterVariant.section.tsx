...
export default function PlanterVariantSection({
  PlantDetails,
  OnChange,
}: PlanterVariantSectionProps) {
  const [availableVariants, setAvailableVariants] = useState<
    AvailablePlanterVariant[]
  >([]);
  ...
  useEffect(() => {
    const getAvailableVariants = async () => {
      try {
        ...
        setError(
          err.response?.data?.message || "Failed to fetch planter variants.",
        );
      }
    };
  ...
  const handleDeletePlanterVariant = async (planterSku: string) => {
    if (!confirm("Are you sure you want to delete this planter variant?"))
      return;
  ...
  const handleDeleteImage = async (planterSku: string, imageUrl: string) => {
    ...
    try {
      ...
      const variant = PlantDetails.planterVariants.find(
  ...
  const uniqueSeries = Array.from(
    new Set(availableVariants.map((v) => v.planterCategory)),
  );
              ...
              <Card sx={{ boxShadow: 3, borderRadius: 2 }}>
                ...
                <CardActions sx={{ justifyContent: "space-between" }}>
                  ...
                  <IconButton
                    color="error"
                    onClick={() =>
                      handleDeletePlanterVariant(variant.planterSku)
                    }
                  >
                    <DeleteIcon />
                  </IconButton>
          ...
          <Stack spacing={3} mt={1}>
            ...
            <FormControl fullWidth>
              ...
              >
                ...
                {/* {Object.values(planterCategoryEnum).map((series) => (
              ...
              >
                {filteredPlanters?.map((planter, idx) => (
                  <MenuItem
                    key={idx}
                    value={[
                      planter.planterCategory,
                      planter.planterSeries,
                      planter.color.name,
                      planter.sku,
                    ]}
                  >
