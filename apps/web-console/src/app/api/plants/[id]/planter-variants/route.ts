...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/plants/${params.id}/planter-variants`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({ message: data.message || "Failed to add planter variant" }), {
    ...
    console.error("Error adding planter variant:", error)
    return new Response(
